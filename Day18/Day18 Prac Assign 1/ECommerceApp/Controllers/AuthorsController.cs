...
public class AuthorsController(BookStoreRepository repository) : ControllerBase
{
    ...
    {
        return repository.UpdateAuthor(id, author) ? NoContent() : NotFound(new { message = $"Author with id {id} was not found." });
    }

    [HttpDelete("{id:int}")]
    public IActionResult DeleteAuthor(int id)
    {
        return repository.DeleteAuthor(id) ? NoContent() : NotFound(new { message = $"Author with id {id} was not found." });
    }
}
