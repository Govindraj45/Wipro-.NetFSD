...
public class MoviesController(MovieCatalogRepository repository) : ControllerBase
{
    ...
    public IActionResult UpdateMovie(int id, Movie movie)
    {
        ...
        return repository.UpdateMovie(id, movie) ? NoContent() : NotFound(new { message = $"Movie with id {id} was not found." });
    }

    [HttpDelete("{id:int}")]
    public IActionResult DeleteMovie(int id)
    {
        return repository.DeleteMovie(id) ? NoContent() : NotFound(new { message = $"Movie with id {id} was not found." });
    }
}
