...
public class DirectorsController(MovieCatalogRepository repository) : ControllerBase
{
    ...
    {
        return repository.UpdateDirector(id, director) ? NoContent() : NotFound(new { message = $"Director with id {id} was not found." });
    }

    [HttpDelete("{id:int}")]
    public IActionResult DeleteDirector(int id)
    {
        return repository.DeleteDirector(id) ? NoContent() : NotFound(new { message = $"Director with id {id} was not found." });
    }
}
